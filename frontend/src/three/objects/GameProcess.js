...
import { lostLiveSound, playSound } from "@components/Audio";
...
class GameProcess {
  ...
  enemyShot(enemy, laser, posX, posY) {
    ...
    if (posX < this.enemyWave.enemiesOnTheScene.length - 1) {
      this.enemyWave.enemiesOnTheScene[posX + 1][posY].setShootingInterval(
  ...
  intersetcts() {
    for (const laser of this.player.lasers) {
      for (let i = 0; i < this.enemyWave.enemiesOnTheScene.length; i++) {
        for (let j = 0; j < this.enemyWave.enemiesOnTheScene[i].length; j++) {
